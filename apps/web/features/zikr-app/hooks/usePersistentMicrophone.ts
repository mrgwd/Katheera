...
export function usePersistentMicrophone({
  onAudioData,
  workletUrl,
}: UsePersistentMicrophoneProps) {
  ...
  // Sync state with service on mount only — start/stop update state
  // directly, so no polling interval is needed.
  useEffect(() => {
